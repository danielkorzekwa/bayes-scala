...
import dk.bayes.gaussian.MultivariateGaussianTest
...
class StaticLocalisationEPTest {
  ...
  @Test def single_observation {
    ...
    val f0 = Gaussian(mu = 3, sigma = 1.5)
    val f1 = LinearGaussian(a = 1, b = 0, sigma = 0.9)
    ...
    assertEquals(1.5, v_given_z.mu, 0.001)
    assertEquals(0.5625, v_given_z.sigma, 0.001)
  }
  ...
  @Test def two_observations {
    ...
    assertEquals(1.153, v_given_z.mu, 0.001)
    assertEquals(0.346, v_given_z.sigma, 0.001)
  }
}