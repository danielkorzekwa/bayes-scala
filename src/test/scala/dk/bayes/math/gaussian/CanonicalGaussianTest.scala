...
object CanonicalGaussianTest {
  ...
  val x = CanonicalGaussian(3, 1.5)

  val a = Matrix(-0.1)
  val yGivenx = CanonicalGaussian(a, 2, 0.5)
}
...
class CanonicalGaussianTest {
  ...
  @Test(expected = classOf[IllegalArgumentException]) def constructor_wrong_dimensions_of_k_and_h {
    new CanonicalGaussian(Matrix(2, 2, Array(0d, 0, 0, 0)), Matrix(0), 0)
  }

  @Test(expected = classOf[IllegalArgumentException]) def constructor_k_matrix_is_not_square {
    CanonicalGaussian(Matrix(3, 2, Array(0d, 0, 0, 0, 0, 0)), Matrix(0, 0, 0))
  }

  @Test(expected = classOf[IllegalArgumentException]) def constructor_h_matrix_is_not_column_vector {
    CanonicalGaussian(Matrix(2, 2, Array(0d, 0, 0, 0)), Matrix(1, 3, Array(0d, 0, 0)))
  }
  ...
  @Test def constructor_linear_gaussian_multivariate {
    ...
    val gaussian = CanonicalGaussian(a = Matrix(2, 2, Array(1d, 0.2, 0.4, 1)), b = Matrix(0.4, 0.7), v = Matrix(2, 2, Array(1.2, 0.4, 5, 2.3)))
    assertEquals(Matrix(-0.279, 1.441, 0.362, 2.338).toString, gaussian.getMean.toString)
    assertEquals(Matrix(4, 4, Array(0.554, 0.138, -0.026, 0.208, 1.662, -0.104, 0.108, -1.229, -0.554, 0.138, -0.009, -0.035, -2.216, -0.277, 0.104, -1.385)).toString, gaussian.getVariance.toString)
  }
  ...
  @Test def pdf {
    ...
    assertEquals(0.398942, CanonicalGaussian(0, 1).pdf(0), 0.0001)
    assertEquals(0.2419, CanonicalGaussian(0, 1).pdf(-1), 0.0001)

    assertEquals(0.10648, CanonicalGaussian(2, 9).pdf(0), 0.0001)
    assertEquals(0.08065, CanonicalGaussian(2, 9).pdf(-1), 0.0001)

    assertEquals(0.03707, CanonicalGaussian(1.65, 0.5).pdf(0), 0.0001)
    assertEquals(0.4607, CanonicalGaussian(1.65, 0.5).pdf(1.2), 0.0001)

    assertEquals(0.0336, CanonicalGaussian(1.7, 0.515).pdf(0), 0.0001)

    assertEquals(0.01111, CanonicalGaussian(Matrix(Array(3, 1.7)), Matrix(2, 2, Array(1.5, -0.15, -0.15, 0.515))).pdf(Matrix(3.5, 0)), 0.0001)
  }
  ...
  @Test def marginalise_from_linear_gaussian_multivariate {
    ...
    val gaussian = CanonicalGaussian(mean, variance)
    ...
    assertEquals(Matrix(0.1, 0.3).toString(), marginal.getMean().toString())
    assertEquals(Matrix(2, 2, Array(2, 0.3, 0.7, 5)).toString(), marginal.getVariance().toString())
  }
